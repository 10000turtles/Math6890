...
/// home/henshw/software/mpich-3.3.1-install/bin/mpiexec -n 2
/// /home/mucelj/Math6890/HW8/heat1d 200 output/temp
...
//--------------------------------------------
inline double getCPU() { return (1.0 * std::clock()) / CLOCKS_PER_SEC; }
...
//--------------------------------------------------------------------------------------
int writeMatlabVector(FILE *matlabFile, Real *u_p, const char *name, int nd1a,
                      int nd1b) {
...
#define un(i) u_p[next][i - nd1a]
  ...
  fprintf(debug_file,
          "-------------------Solvetheheatequationin1Dsolution=%s--------------"
          "-------\n",
          solutionName);
  fprintf(debug_file, "numGhost=%d,n1a=%d,n1b=%d,nd1a=%d,nd1b=%d\n", numGhost,
          n1a, n1b, nd1a, nd1b);
  fprintf(debug_file,
          "numSteps=%d,Nx=%d,kappa=%g,tFinal=%g,boundaryCondition(0,0)=%d,"
          "boundaryCondition(1,0)=%d\n",
          numSteps, Nx, kappa, tFinal, boundaryCondition(0, 0),
  ...
  for (int n = 0; n < numSteps; n++) {
    ...
    for (int i = n1a; i <= n1b; i++) {
      un(i) = uc(i) + rx * (uc(i + 1) - 2. * uc(i) + uc(i - 1)) +
              dt * FORCE(x(i), t);
    }
    ...
    if (rank == 0) {
      ...
      if (boundaryCondition(side, 0) == dirichlet) {
        un(i) = UTRUE(x(i), t + dt);
        un(i - is) =
            3. * un(i) - 3. * un(i + is) + un(i + 2 * is); // extrapolateghost
      } else {
        // NeumannBC
        un(i - is) = un(i + is) - 2. * is * dx * UTRUEX(x(i), t + dt);
      }
      if (num_processors > 1)
        if (is_blocked) {
          ...
          // Receive from rank 1
          MPI_Recv(&un(n1b + 1), 1, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD,
                   MPI_STATUS_IGNORE);
          if (debug > 0) {
          ...
          // Receive from rank 1
          MPI_Irecv(&un(n1b + 1), 1, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD,
                    &receive_right);
          if (debug > 0) {
            fprintf(debug_file, "Receiving un(%d) = %f from rank %d\n", n1b + 1,
                    un(n1b + 1), 1);
    ...
    if (rank == num_processors - 1) {
      ...
      if (num_processors > 1)
        if (is_blocked) {
          ...
          MPI_Recv(&un(n1a - 1), 1, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD,
                   MPI_STATUS_IGNORE);
          if (debug > 0) {
            fprintf(debug_file, "Receiving un(%d) = %f from rank %d\n", n1a - 1,
                    un(n1a - 1), rank - 1);
          ...
          // Receive from rank num_p-2
          MPI_Irecv(&un(n1a - 1), 1, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD,
                    &receive_left);
          if (debug > 0) {
            fprintf(debug_file, "Receiving un(%d) = %f from rank %d\n", n1a - 1,
                    un(n1a - 1), rank - 1);
          ...
          // Send to rank num_p-2
          MPI_Isend(&un(n1a), 1, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD,
                    &send_left);
          if (debug > 0) {
            fprintf(debug_file, "Sending un(%d) = %f to rank %d\n", n1a,
                    un(n1a), rank - 1);
    ...
    if (rank != 0 && rank != num_processors - 1) {
      if (is_blocked) {
        ...
        MPI_Recv(&un(n1b + 1), 1, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE);
        if (debug > 0) {
          fprintf(debug_file, "Receiving un(%d) = %f from rank %d\n", n1b + 1,
                  un(n1b + 1), rank + 1);
        ...
        // Send to rank "right"
        MPI_Isend(&un(n1b), 1, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD,
                  &send_right);
        if (debug > 0) {
          fprintf(debug_file, "Sending un(%d) = %f to rank %d\n", n1b, un(n1b),
                  rank + 1);
        ...
        // Receive from rank "right"
        MPI_Irecv(&un(n1b + 1), 1, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD,
                  &receive_right);
        if (debug > 0) {
          fprintf(debug_file, "Receiving un(%d) = %f from rank %d\n", n1b + 1,
                  un(n1b + 1), rank + 1);
  ...
  if (rank == 0) {
    ...
    for (int i = 0; i < num_processors - 1; i++) {
      ...
      MPI_Recv(&max_cpu, 1, MPI_DOUBLE, i + 1, tag + i + 1, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
...
#define error(i) error_p[i - nd1a]
  ...
  if (rank == 0) {
    ...
    fprintf(matlabFile,
            "xa=%g;xb=%g;kappa=%g;t=%g;maxErr=%10.3e;cpuTimeStep=%10.3e;\n", xa,
            xb, kappa, tFinal, maxErr, cpuTimeStep);

    fprintf(debug_file, "max_cpu = %f\n", max_cpu);

    fprintf(matlabFile,
            "Nx=%d;dx=%14.6e;numGhost=%d;n1a=%d;n1b=%d;nd1a=%d;nd1b=%d;\n", Nx,
            dx, numGhost, n1a, n1b, nd1a, nd1b);
  ...
  if (rank == 0) {
    ...
    for (int i = 0; i < num_processors - 1; i++) {
      ...
      MPI_Recv(&maxErr, 1, MPI_DOUBLE, i + 1, tag + i + 1, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE);
